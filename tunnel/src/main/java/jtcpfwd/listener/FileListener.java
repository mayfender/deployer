...
public class FileListener extends Listener {
	...
	private final List/* <String> */knownEntries = new ArrayList();
	...
	protected Socket tryAccept() throws IOException {
		while (!disposed) {
			...
			for (int i = 0; i < entries.length; i++) {
				if (knownEntries.contains(entries[i]))
					continue;
				knownEntries.add(entries[i]);
				if (!new File(directory, entries[i]).isDirectory())
					continue;
	...
	private Socket buildSocket(File basedir) throws IOException {
		if (!new File(basedir, "ack").createNewFile())
			throw new IOException("Unable to create Ack file");
