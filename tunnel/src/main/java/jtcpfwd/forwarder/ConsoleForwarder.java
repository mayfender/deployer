...
public class ConsoleForwarder extends Forwarder {
	...
	public static final Class[] getRequiredClasses() {
		return new Class[] {
				Lookup.class,
				ForwarderThread.class,
				ForwarderHandlerThread.class,
				StreamForwarder.class,
				Listener.class,
				NoMoreSocketsException.class,
				SplitOutputStream.class };
	}
	...
	private static final String[] commandDescriptions = new String[] {
			"Show this help screen",
			"Exit console mode",
			"Restart jTCPfwd",
			"Show accumulated log buffer",
			"List currently configured listeners/forwarders",
			"Add a new listener and forwarder",
			"Remove listener/forwarder by index",
			"Activate added listener/forwarder by index"
	};

	private final boolean restartAllowed;
	private final List/* <String> */rules;
	private final List/* <String> */readOnlyRules = new ArrayList();
	private final List/* <ForwarderThread> */startedForwarderThreads = new ArrayList();
	...
	public ConsoleForwarder(String rule) throws Exception {
		...
		List/* <String> */params = new ArrayList(Arrays.asList(rule.split(",")));
	...
	public Module[] getUsedModules() {
		// this method is needed due to a false positive in
		// CustomLiteBuilder.
			...
			while (true) {
				...
				case '?':
					...
					for (int i = 0; i < shortCommands.length(); i++) {
						if (allowedCommands.indexOf(shortCommands.charAt(i)) != -1)
							bw.write("\t" + shortCommands.charAt(i) + "\t" + longCommands.get(i) + "\t" + commandDescriptions[i] + "\r\n");
					}
