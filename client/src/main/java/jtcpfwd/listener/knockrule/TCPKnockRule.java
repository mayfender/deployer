...
public class TCPKnockRule extends PatternBasedKnockRule implements Runnable {
	...
	public static final Class[] getRequiredClasses() {
		return new Class[] {
				PatternBasedKnockRule.class,
				BinaryExpression.class, MaskedBinaryExpression.class,
	...
	public void trigger(InetAddress peer) throws IOException {
		...
		if (response != null) {
			...
			if (!Arrays.equals(response, bytes))
				throw new IOException("Trigger failed");
		}
		s.close();
	}
		...
		public void run() {
			try {
				try {
					...
					if (in.available() == 0 && rule.check(s, baos.toByteArray()))
						return;
