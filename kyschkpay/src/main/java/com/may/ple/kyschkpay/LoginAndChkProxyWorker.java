...
public class LoginAndChkProxyWorker implements Runnable {
	private static final Logger LOG = Logger.getLogger(LoginAndChkProxyWorker.class.getName());
	private List<UpdateChkLstModel> loginList = new ArrayList<>();
	private final int LIMITED_UPDATE_SIZE = 10;
	private List<LoginAndChkWorkerModel> worker;
	...
	public LoginAndChkProxyWorker(ThreadPoolExecutor executor, String token, String proxyStr, List<LoginAndChkWorkerModel> worker) {
	...
	public void run() {
		try {
			...
			for (LoginAndChkWorkerModel loginWorkerModel : worker) {
				key = loginWorkerModel.getProductId()+"#"+proxyStr+"#"+loanType;
				secondLogin = ManageLoginAndChkWorkerThread.firstLoginMap.get(key);
				...
				executor.execute(new LoginAndChkWorker(this, proxy, loginWorkerModel, secondLogin));
			}
